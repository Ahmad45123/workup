...
public class SearchJobsCommand extends JobCommand<SearchJobsRequest, SearchJobsResponse> {
  ...
  public SearchJobsResponse Run(SearchJobsRequest request) {
    try {
      ...
      if (request.getPagingState() != null) {
        ...
        ByteBuffer byteBuffer = com.datastax.oss.protocol.internal.util.Bytes.fromHexString(
      ...
      Slice<Job> result = jobRepository.searchForJob(
        "%" + request.getQuery() + "%",
        pageRequest
      );
  ...
  private static <T> String getPagingState(final Slice<T> slice) {
    if (slice.hasNext()) {
      ...
      String pagingStateStr = com.datastax.oss.protocol.internal.util.Bytes.toHexString(
        pageRequest.getPagingState()
      );
