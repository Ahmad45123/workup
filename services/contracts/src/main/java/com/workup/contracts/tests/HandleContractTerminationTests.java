...
public class HandleContractTerminationTests {
  ...
  public void requestNotFoundTest(AmqpTemplate template) {
    ...
    HandleTerminationRequest request = HandleTerminationRequest
      .builder()
      .withContractTerminationRequestId(UUID.randomUUID().toString())
      .withChosenStatus(TerminationRequestStatus.REJECTED)
      .build();

    HandleTerminationResponse response = (HandleTerminationResponse) template.convertSendAndReceive(
      "contractsqueue",
      request
    );
    assert response != null;
    if (
      response.getErrorMessage().equals("No Termination Requests Found")
    ) System.out.println(
      " [x] Request Not Found Test has Passed"
    ); else System.out.println(" [x] Request Not Found Test has Failed");

    System.out.println(
      "[x] Finished RequestContractTermination Request NotFound Test .....\n"
    );
  }
  ...
  public void successTest(AmqpTemplate template) throws ParseException {
    ...
    ContractTerminationRequest terminationRequest = ContractTerminationRequest
      .builder()
      .withUserId(clientId)
    ...
    ContractTerminationResponse terminationResponse = (ContractTerminationResponse) template.convertSendAndReceive(
      "contractsqueue",
      terminationRequest
    );
