...
public interface PaymentTransactionRepository
  ...
  @Query(
    value = "SELECT * FROM PaymentTransaction pt WHERE pt.payment_request_id IN (SELECT pr.id FROM PaymentRequest pr WHERE pr.freelancer_id = ?1)",
    nativeQuery = true
  )
  List<PaymentTransaction> findAllByFreelancerId(String freelancerId);

  @Query(
    value = "SELECT * FROM PaymentTransaction pt WHERE pt.payment_request_id IN (SELECT pr.id FROM PaymentRequest pr WHERE pr.client_id = ?1)",
    nativeQuery = true
  )
